...
impl Orchestrator {
    ...
    async fn reconcile_stale_pods(&mut self) {
        ...
        let mut to_reconcile: HashSet<Uuid> = HashSet::new();
        for pod_id in running_pod_ids {
            let (container_id, deployment_id) = match self.pods.get(&pod_id) {
                Some(p) => match &p.container_id {
                    Some(cid) => (cid.clone(), p.deployment_id),
                    None => continue,
                },
                None => continue,
            };
            ...
            // The container is gone (typically the host rebooted). Honour the restart
            // policy: recreate the pod for Always/OnFailure, leave Failed for Never.
            let policy = self
                .deployments
                .get(&deployment_id)
                .map(|d| d.spec.restart.clone())
                .unwrap_or_default();
            if matches!(policy, RestartPolicy::Never) {
                if let Some(pod) = self.pods.get_mut(&pod_id) {
                    tracing::warn!(pod_id = %pod_id, container_id = %container_id, "pod container not running, marking Failed (restart=never)");
                    pod.status = PodStatus::Failed;
                    let cloned = pod.clone();
                    self.persist_update_pod(&cloned).await;
                }
            } else {
                tracing::warn!(pod_id = %pod_id, container_id = %container_id, "pod container not running, recreating");
                self.cleanup_dead_pod(pod_id, &container_id).await;
                to_reconcile.insert(deployment_id);
            }
        }
        // Recreate missing pods and refresh proxy routes for affected deployments.
        for deployment_id in to_reconcile {
            if let Err(e) = self.reconcile_deployment(deployment_id).await {
                tracing::warn!(deployment_id = %deployment_id, error = %e, "failed to reconcile deployment after reboot");
            }
        }
    ...
    /// Tear down a pod whose container is gone: drop health tracking, deregister DNS,
    /// remove the dead container, and forget the pod so a reconcile recreates it.
    async fn cleanup_dead_pod(&mut self, pod_id: Uuid, container_id: &str) {
        self.health_tracker.unregister(&pod_id);
        let info = self.pods.get(&pod_id).map(|p| {
            (
                p.project.clone(),
                p.deployment_name.clone(),
                p.container_ip.clone(),
            )
        });
        if let Some((project, deployment_name, container_ip)) = info {
            if let (Some(dns), Some(ip_str)) = (&self.dns, &container_ip) {
                if let Ok(ip) = ip_str.parse::<std::net::IpAddr>() {
                    let _ = dns.deregister(&project, &deployment_name, ip).await;
                }
            }
        }
    ...
    /// If the spec requests public networking with a domain, ensure a matching route
    /// exists in the store (idempotent). The proxy config itself is (re)generated by the
    /// reconcile that follows. Falls back to HTTP-only if `https` is set but no ACME
    /// email is configured, so the deploy still succeeds.
    async fn ensure_network_route(&mut self, spec: &DeploymentSpec) {
        let net = match &spec.network {
            Some(n) if n.public => n,
            _ => return,
        };
        let domain = match net.domain.as_deref() {
            Some(d) if !d.is_empty() => d.to_string(),
            _ => return,
        };
        let has_email = !self.acme_email.as_deref().unwrap_or("").trim().is_empty();
        let tls_mode = if net.https && has_email {
            TlsMode::Auto
        } else {
            if net.https && !has_email {
                tracing::warn!(
                    domain,
                    "network.https requested but no --acme-email; creating HTTP-only route"
                );
            }
            TlsMode::None
        };
        if let Some(store) = &self.route_store {
            let existing = store.list_routes(None).await.unwrap_or_default();
            if !existing.iter().any(|r| r.domain == domain) {
                let route = Route::new(&domain, &spec.project, &spec.deployment.name, tls_mode);
                if let Err(e) = store.insert_route(&route).await {
                    tracing::warn!(domain, error = %e, "failed to persist network route");
                }
            }
        }
    }
    ...
    async fn handle_deploy(&mut self, spec: DeploymentSpec) -> Result<Deployment> {
        ...
        self.ensure_network_route(&spec).await;
    ...
    async fn reconcile_deployment(&mut self, deployment_id: Uuid) -> Result<()> {
        ...
        // Keep the proxy config in sync with the (possibly new) pod IPs.
        self.reapply_routes().await;
    ...
    fn build_container_config(
        ...
        // Public deployments are reached through the reverse proxy over the project
        // network, so we don't also bind their ports on the host (which would collide
        // with the proxy on 80/443). Single-replica private deployments still publish.
        let is_public = spec.network.as_ref().map(|n| n.public).unwrap_or(false);
        let ports = spec
            .ports
            .iter()
            .map(|&p| crate::ports::runtime::PortBinding {
                container_port: p,
                host_port: if spec.replicas == 1 && !is_public {
                    Some(p)
                } else {
                    None
                },
    ...
    async fn handle_add_route(
        ...
        // Automatic TLS needs an ACME contact email, configured on the daemon via
        // `--acme-email`. Validate against the daemon's configured email instead of
        // rejecting unconditionally.
        if tls_mode == TlsMode::Auto && self.acme_email.as_deref().unwrap_or("").trim().is_empty() {
            return Err(HelyosError::InvalidSpec(
                "TLS mode 'auto' requires an ACME email; start helyosd with --acme-email".into(),
            ));
        }

        let route = Route::new(domain, project, deployment, tls_mode);

        if let Some(store) = &self.route_store {
            store.insert_route(&route).await?;
        }

        // Regenerate the full proxy config from the store + current pods so the file
        // always reflects every route with up-to-date upstream IPs.
        self.reapply_routes().await;
    ...
    async fn handle_remove_route(&mut self, domain: &str) -> Result<()> {
        ...
        // Rewrite the proxy config from the remaining routes (atomic full overwrite).
        self.reapply_routes().await;
    ...
    /// Rebuild and apply the entire proxy configuration from the route store and the
    /// current running pods. Idempotent — safe to call on every route change and after
    /// each deployment reconcile, which keeps upstream IPs fresh as pods are recreated.
    async fn reapply_routes(&self) {
        let (proxy, store) = match (&self.proxy, &self.route_store) {
            (Some(p), Some(s)) => (p, s),
            _ => return,
        };
        let routes = store.list_routes(None).await.unwrap_or_default();

        let configs: Vec<RouteConfig> = routes
            .iter()
            .map(|route| {
                // Upstream port: the deployment's first declared port, else 80.
                let port = self
                    .deployment_index
                    .get(&(route.project.clone(), route.deployment.clone()))
                    .and_then(|id| self.deployments.get(id))
                    .and_then(|d| d.spec.ports.first().copied())
                    .unwrap_or(80);

                let upstream: Vec<ProxyUpstream> = self
                    .pods
                    .values()
                    .filter(|p| {
                        p.project == route.project
                            && p.deployment_name == route.deployment
                            && p.status == PodStatus::Running
                    })
                    .filter_map(|p| {
                ...
                let tls = match route.tls_mode {
                    TlsMode::None => ProxyTlsConfig::None,
                    TlsMode::Auto => ProxyTlsConfig::Auto {
                        email: self.acme_email.clone().unwrap_or_default(),
                    },
        ...
        if let Err(e) = proxy.apply_routes(&configs).await {
            tracing::warn!(error = %e, "failed to apply proxy routes");
        }
        if let Err(e) = proxy.reload().await {
            tracing::warn!(error = %e, "failed to reload proxy");
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn reconcile_recreates_stale_pods_for_always_policy() {
        let store = Arc::new(InMemoryStore::new());
        ...
        // ConfigurableMockRuntime has no knowledge of "vanished-container", so the pod's
        // container looks gone (as after a host reboot). With restart=always the pod must
        // be recreated, not left Failed.
    ...
    #[tokio::test]
    async fn reconcile_marks_stale_pods_failed_for_never_policy() {
        let store = Arc::new(InMemoryStore::new());
        let project = Project::new("myapp");
        store.insert_project(&project).await.unwrap();
        ...
        // restart=never: the pod stays Failed and the deployment is Degraded.
        let pods = handle.list_pods(None).await;
        assert_eq!(pods.len(), 1);
